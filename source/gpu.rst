...
**SISD, SIMD, and SPMD Pipelines**

This section illustrates the difference between SISD, SIMD, and SPMD
...
There is no mainstream GPU that is truly hardware-SPMD (where each thread has 
...
.. code-block::
   ...
   Execution Model (SIMD/SIMT hardware):
   -------------------------------------
       Warp/Wavefront (e.g., 32 or 64 threads in a group)

       [F] Fetch    One instruction fetched for the entire group
       [D] Decode   Decoded once, applied to all active lanes
       [E] Execute  ALUs operate in parallel lanes (lock-step)
       [M] Memory   Loads/stores from each lane's address
       [W] Writeback Results written per lane

       Example in lock-step:
           I1: LD R1, A[tid]   → executed for all threads in warp
           I2: LD R2, B[tid]   → executed for all threads in warp
           I3: ADD R3, R1, R2  → executed in parallel lanes
           I4: ST C[tid], R3   → executed in parallel lanes

   Divergence Handling (SIMT):
   ----------------------------
       If threads branch differently:

           if (tid % 2 == 0) { A[...] }
           else              { B[...] }

       The warp executes both paths sequentially:
           - First with even lanes active, odd lanes masked off
           - Then with odd lanes active, even lanes masked off
       → Produces the *illusion* of independent threads (SPMD),
         but still reuses SIMD hardware underneath.
