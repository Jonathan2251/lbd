...
Comparing traditional vs mesh‑shader pipelines is shown as the following 
:numref:`short-rendering-pipeline-shaders-compare` and table:


.. _short-rendering-pipeline-shaders-compare: 
.. graphviz:: ../Fig/gpu/short-rendering-pipeline-shaders-compare.gv
  :caption: GPU Pipeline For Shaders (Traditional vs Mesh-Shader)

.. list-table:: summarizing all shader stages
   :header-rows: 1
   :widths: 18 25 40 17
   ...
   * - Vertex Shader
     - Traditional
     - Processes per-vertex attributes from vertex input (positions, normals,
       UVs, etc.), applies transforms, may do basic skinning or morphing.
     - No
   * - Tessellation Control Shader
     - Traditional
     - Controls tessellation factors, prepares patch data for the tessellation
       evaluation shader.
     - Yes
   * - Tessellation Evaluation Shader
     - Traditional
     - Evaluates tessellated vertices on patches (curves/surfaces), often used
       for displacement and smooth surfaces.
     - Yes
   * - Geometry Shader
     - Traditional
     - Operates on whole primitives (triangles, lines, points), can expand,
       shrink, or modify them, often used for simple amplification or effects.
     - Yes
   * - Fragment Shader
     - Both
     - Runs per-fragment after rasterization, performs texturing, lighting,
       shading, material evaluation, and writes colors/depth to the framebuffer.
     - No
   * - Compute Shader
     - Both
     - General-purpose compute stage, independent of the fixed graphics
       pipeline. Used for skinning, culling, particles, post-processing,
       and other GPGPU tasks.
     - Yes
   * - Task Shader
     - Mesh-Shader Pipeline
     - Work-dispatch stage that runs in workgroups, performs coarse culling,
       LOD selection, and determines how many mesh shader workgroups to launch.
       Does not directly emit primitives.
     - Yes
   * - Mesh Shader
     - Mesh-Shader Pipeline
     - Compute-like stage that generates vertices and primitives directly in
       workgroups, replacing vertex input, vertex shader, and often tessellation
       and geometry stages. Operates naturally on meshlets.
     - No (if mesh pipeline is used)


**GPU Architecture–Level Explanation of Meshlets and Work Distribution**

**Overview**

Mesh shaders introduce a compute-like geometry pipeline that replaces
the traditional vertex input, vertex shader, tessellation, and often
geometry shader stages. At the architectural level, this pipeline is
organized around *meshlets* and *workgroups*, enabling efficient
GPU-driven rendering, fine-grained culling, and improved cache locality.

**What Meshlets Are**

A **meshlet** is a compact cluster of vertices and triangles extracted
from a larger mesh. Meshlets are designed to fit efficiently into GPU
workgroup memory and caches.

Characteristics:

- Small, bounded size (e.g., up to ~256 vertices and ~256 triangles).
- High spatial and topological locality.
- Preprocessed offline or at load time using specialized
  meshlet-generation tools.
- Ideal for GPU workgroup processing because all data needed for a
  meshlet fits into fast on-chip memory.

Meshlets serve as the fundamental unit of geometry work in the
mesh-shader pipeline.

**Work Distribution in the Mesh-Shader Pipeline**

The mesh-shader pipeline consists of two programmable stages:

1. **Task Shader (Optional)**
2. **Mesh Shader**

These stages operate similarly to compute shaders, using workgroups and
shared memory.
...
Meshlets are always generated later, using specialized meshlet‑generation tools, most commonly:
...
- in: Receives per-vertex attributes from buffer objects, it is 
  **3D Model Information** described in :numref:`in-out-rendering`.
- out: Passes data to next stage (e.g., fragment shader)
- **uniform**: Global parameters like matrices or lighting, it is 
  **Uniform Updates** described in :numref:`in-out-rendering`.
